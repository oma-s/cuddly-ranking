...
export default function ToysIndexScreen() {
  ...
  const renderItem = ({ item }: { item: (typeof toys)[number] }) => (
    <ToyListItem toy={item} onPress={() => router.push({ pathname: '/(tabs)/toys/[id]', params: { id: item.id } })} />
  );
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <View>
              <Text style={[styles.overline, { color: theme.tint }]}>Your Crew</Text>
              <Text style={[styles.title, { color: theme.text }]}>Cuddly Ranking</Text>
              <Text style={[styles.subtitle, { color: `${theme.text}aa` }]}>
                Tap a toy to view stats, or add a new recruit.
              </Text>
            </View>
            <Pressable
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            {loading ? (
              ...
              <>
                <Text style={[styles.emptyTitle, { color: theme.text }]}>No toys yet</Text>
                <Text style={[styles.emptySubtitle, { color: `${theme.text}80` }]}>
                  Start building your crew by adding the first hero.
                </Text>
              </>
            )}
          </View>
